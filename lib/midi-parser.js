...
function parseMidi(data, opts) {
  opts = opts || {}

  var p = new Parser(data, opts)

  var headerChunk = p.readChunk()
  if (headerChunk.id != 'MThd')
    throw "Bad MIDI file.  Expected 'MHdr', got: '" + headerChunk.id + "'"
  var header = parseHeader(headerChunk.data, opts)
  ...
  for (var i=0; !p.eof() && i < header.numTracks; i++) {
    ...
    var track = parseTrack(trackChunk.data, opts)
    tracks.push(track)
  }
...
function parseHeader(data, opts) {
  var p = new Parser(data, opts)
...
Parser.prototype.readId = function(len) {
